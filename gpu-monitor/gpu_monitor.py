...
def get_gpu_infos(nvidiasmi_output):
    ...
    for idx, gpu in enumerate(gpus):
        ...
        mems = gpu.findall('fb_memory_usage')
        mem_used = [mem.find('used').text for mem in mems][0]
        temps = gpu.findall('temperature')
        gpu_temp = [temp.find('gpu_temp').text for temp in temps][0]
        gpu_infos.append({'idx': idx, 'model': model, 'pids': pids, 'mem_used' : mem_used, 'gpu_temp' : gpu_temp})
